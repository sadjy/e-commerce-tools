...
export async function setupApi() {
	...
	const tmpAuthenticationService = new AuthenticationService(identityService, ssiService, {
	...
	if (!serverIdentityId || !serverIdentity) {
		...
		const authenticationService = new AuthenticationService(identityService, ssiService, {
			jwtExpiration: '2 days',
			serverSecret,
			serverIdentityId: identity.doc.id
		});

		const serverUser = await ssiService.getUser(identity.doc.id);
		if (!serverUser) {
