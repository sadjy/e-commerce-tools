...
describe('test authentication routes', () => {
	...
	let ssiService: SelfSovereignIdentityService;
	...
	beforeEach(() => {
		...
		authenticationService = new AuthenticationService(identityService, ssiService, {
			jwtExpiration: '2 days',
			serverSecret,
			serverIdentityId: ServerIdentityMock.doc.id
		});
		authenticationRoutes = new AuthenticationRoutes(authenticationService, ssiService, authorizationService, config);
	...
	describe('test create-identity route', () => {
		it('should send result for valid body', async () => {
			...
			const userSpy = spyOn(ssiService, 'addUser').and.returnValue({ result: { n: 1 } });
			const req: any = {
				params: {},
				body: {
	...
	describe('test getNonce route', () => {
		it('should return bad request because no userId provided.', async () => {
			...
			const getUserSpy = spyOn(ssiService, 'getUser').and.returnValue(userMock);
		...
		it('should return a valid nonce to solve', async () => {
			...
			const getUserSpy = spyOn(ssiService, 'getUser').and.returnValue(validUserMock);
			const upsertNonceSpy = spyOn(AuthDb, 'upsertNonce');
			const req: any = {
				params: { userId },
				body: null
